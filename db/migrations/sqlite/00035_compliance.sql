...
--
-- Deviations, as elsewhere in this directory: TEXT ids and timestamps, INTEGER
-- for keys and booleans, and full indexes where Postgres uses partial ones.
-- SQLite supports partial indexes, but the planner uses them less predictably
-- and the tables here are small enough on a developer's machine that the
-- difference is not worth a second shape to keep in step.
...
CREATE INDEX compliance_runs_running ON compliance_runs (state, heartbeat_at);
...
CREATE TABLE compliance_charts (
    run_id          TEXT    NOT NULL REFERENCES compliance_runs(id) ON DELETE CASCADE,
    name            TEXT    NOT NULL,
...
CREATE INDEX compliance_results_findings
    ON compliance_results (run_id, outcome, severity, check_id);
...
CREATE TABLE package_compliance (
    ...
    run_id          TEXT REFERENCES compliance_runs(id) ON DELETE SET NULL,
    state           TEXT        NOT NULL DEFAULT ''
                      CHECK (state IN ('','running','complete','failed','cancelled')),
    verdict         TEXT        NOT NULL DEFAULT '',
    blocking_count  INTEGER     NOT NULL DEFAULT 0,
