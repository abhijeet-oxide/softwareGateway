...
one. Both work; the console has two traps, in steps 2 and 4.
...
4. Create, then grant them a role **on the `platform` project** - its keys are
   named `<product>:<role>` for product roles. Granting on the product's own
   project instead is the one mistake that looks right: no token carries roles
   for that project, so the console shows the grant and the person signs in
   holding nothing. Or add them to `config/users/users.yaml` and re-run the
   seeder, which is the same grant in a file somebody can review.
