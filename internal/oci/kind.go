...
// Classify names what an artifact is from the two fields that describe it.
//
// Both may be empty, and that is a real state rather than a caller's mistake —
// content older than OCI 1.1 has no `artifactType`, and a descriptor written by
// something careless has neither.
func Classify(mediaType, artifactType string) string {
	switch {
