...
{{- $url := index $.Values.access (printf "%sUrl" $which) }}
{{- if not (regexMatch "^https?://[^/]+$" $url) }}
{{- fail (printf "\n\naccess.%sUrl is %q.\n\nIt must be the whole address a browser uses and nothing more: a scheme, a host\nand an optional port, with no path and no trailing slash. A hostname or an IP,\nwith or without DNS.\n\n  http://10.20.30.40:8000\n  https://gateway.example.internal\n" $which $url) }}
{{- end }}
{{- end }}
...
{{- range $which := list "web" "identity" }}
{{- $browserPort := int (include "swgw.urlPort" (index $.Values.access (printf "%sUrl" $which))) }}
{{- $port := int (default $browserPort (index $.Values.access.expose.nodePort $which)) }}
{{- if or (lt $port 30000) (gt $port 32767) }}
{{- fail (printf "\n\nThe %s node port resolves to %d, which is outside the default NodePort range\n30000-32767.\n\nWith access.expose.type nodePort the browser's port IS the node port, so put one\nin that range in access.%sUrl - or set access.expose.nodePort.%s to the node port\nand leave access.%sUrl as the address a browser reaches through whatever sits in\nfront of it.\n" $which $port $which $which $which) }}
{{- end }}
{{- end }}
{{- end }}
