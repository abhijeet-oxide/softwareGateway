...
| `BUILD_INFO` | cd | The release is not published to Artifactory's Builds view. Set it to `true` and a release becomes one object you can promote and scan, published with the JFrog CLI. See [docs/design/32](../../docs/design/32-release-build-info.md). |
| `BUILD_INFO_URL` | cd | Derived as `https://<REGISTRY_HOST>/artifactory`. Set it only for an instance serving its registry on a different hostname than its API. |
| `BUILD_INFO_PROJECT` | cd | No project key is sent, so the build is filed in Artifactory's **global** scope - accepted, and not visible under the project your repositories belong to. If a repository is named `<key>-oci-stage`, that prefix is the key. |
