...
| SUP-11 `NEW` | T1-R | v2 | block | No image uses a moving label - `latest`, `stable`, `main`, `master`, `edge`, `dev`, `test`, `nightly` - or no tag at all. This is the prohibited half, and separating it lets a vendor fix one without being told about the other. |
